...
$ID = ($ID == null) ? 1 : $ID;
$sql = "SELECT id, jobName, jobImage FROM jobs WHERE id='$ID'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
if ($result->num_rows > 0) {
    // mostro i dati nella pagina
    printf("<h1>%s</h1> <div id='align'><img alt='%s' width='300' title='%s' src='%s'></div>", $row["jobName"], $row["jobName"], $row["jobName"], $row["jobImage"]);
    }
else {
    echo "0 risultati.";
}
